...
abstract class NasaRepositoryContract {
  ...
  NasaRepositoryContract({required NasaHttpService httpService})
...
class NasaRepository implements NasaRepositoryContract {
  ...
  NasaRepository({required NasaHttpService httpService})
      : _httpService = httpService;

  @override
  Future<APODNasaModel> getAstronomyPictureOfTheDay({DateTime? date}) async {
    var response =
        await _httpService.get(path: '/planetary/apod', queryParams: {
